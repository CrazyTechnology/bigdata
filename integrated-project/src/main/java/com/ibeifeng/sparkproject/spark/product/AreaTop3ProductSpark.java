 ...
 **spark sql 支持两种方式将RDD转换为dataFrame
 * 第一种方式：
 * 	使用反射来推断包含了特定数据类型的RDD的元数据。这种基于反射的方式，代码比价简洁，
 * 第二种方式：
 * 	是通过编程接口来创建dataframe ,可以在程序运行时动态的构成一份元数据，然后应用到已经存在的rdd中。
 * 	这种方式的代码比价冗余，但是如果在编写程序时还不知道RDD的元数据，只有在程序运行时才知道。
 * 	那么只能通过这种动态构建元数据的方式。
 */
		...
		JavaRDD<Row> mappedRDD = joinedRDD.map(
				...
				new Function<Tuple2<Long,Tuple2<Row,Row>>, Row>() {
					...
					public Row call(Tuple2<Long, Tuple2<Row, Row>> tuple)
						...
						//clickAction信息，模拟数据生成的session信息
